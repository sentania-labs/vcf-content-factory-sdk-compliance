...
public final class ControlEvaluator {
	...
	/**
	 * v3 (build 57): minimum virtual-hardware-version mode. An expected
	 * value of the form {@code vmx-N or higher} / {@code vmx-N or newer}
	 * (SCG 9.1 and SCG 7.0 phrasing for {@code vm.virtual-hardware}) is a
	 * floor, not a literal: a VM reading {@code vmx-M} is compliant iff
	 * {@code M >= N}. A present value that is not of the form
	 * {@code vmx-<digits>} is non-compliant (never a guess-pass); an absent
	 * or unreadable value never reaches this helper (short-circuited by the
	 * callers, exactly like the other modes). A bare {@code vmx-N} (SCG 8.0
	 * and 9.0) keeps exact equality: those rows' descriptions document the
	 * exact-match semantic and are regenerated by the factory normalizer,
	 * not this repo.
	 */
	static final java.util.regex.Pattern VMX_MINIMUM = java.util.regex.Pattern
			.compile("(?i)^vmx-(\\d+)\\s+or\\s+(?:higher|newer)$");

	static final java.util.regex.Pattern VMX_VALUE = java.util.regex.Pattern
			.compile("(?i)^vmx-(\\d+)$");
	...
	public static ComplianceResult evaluateControls(
		...
		for (BenchmarkProfile.Control control : controls) {
			...
			} else if (isVmxMinimumMode(expected)) {
				compliant = vmxMinimumMatches(actual, expected);
			} else {
	...
	static boolean vimPropertyMatches(Object actual, String expected) {
		...
		if (isVmxMinimumMode(expected)) {
			return vmxMinimumMatches(actual, expected);
		}
	...
	/** True when {@code expected} is a {@code vmx-N or higher|newer} floor. */
	static boolean isVmxMinimumMode(String expected) {
		if (expected == null) return false;
		return VMX_MINIMUM.matcher(stripQuotes(expected.trim())).matches();
	}

	/**
	 * Minimum-hardware-version comparison. Caller guarantees {@code actual}
	 * is a present read value. Compliant iff actual is {@code vmx-M} with
	 * {@code M >= N}; anything unparseable is non-compliant.
	 */
	static boolean vmxMinimumMatches(Object actual, String expected) {
		if (actual == null || expected == null) return false;
		java.util.regex.Matcher e = VMX_MINIMUM.matcher(
				stripQuotes(expected.trim()));
		java.util.regex.Matcher a = VMX_VALUE.matcher(
				stripQuotes(String.valueOf(actual).trim()));
		if (!e.matches() || !a.matches()) return false;
		try {
			return Integer.parseInt(a.group(1)) >= Integer.parseInt(e.group(1));
	...
	public static final class ControlResult {
		...
		// v3 (build 57): explicit unreadable marker. An unreadable control
		// is never compliant, and it is not a failure either: the push
		// writes Compliant=-1 for it (not 0), so the per-control
		// "Compliant == 0" compliance alert never fires a remediation
		// runbook for a setting the adapter could not read.
		...
		public ControlResult(String scgId, String actual, String expected,
			...
			this(scgId, actual, expected, compliant, description, false);
		}

		public ControlResult(String scgId, String actual, String expected,
				boolean compliant, String description, boolean unreadable) {
			this.scgId = scgId;
			this.actual = actual;
			this.expected = expected;
			this.compliant = compliant && !unreadable;
