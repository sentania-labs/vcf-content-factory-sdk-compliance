#!/usr/bin/env python3
"""Normalize the VMware SCG 8.0 source CSV to the canonical schema.

Usage (run from the factory checkout the adapter repo is cloned into):
  python3 scripts/normalize_scg_v80.py \\
      profiles/vmware_scg_8.0.csv profiles/canonical/scg_8.0.csv

A thin driver over the factory's SCG 8 normalizer
(`<factory>/scripts/normalize_scg_v8.py`), added in adapter build 72 so
adapter-side corrections to the 8.0 profile regenerate reproducibly.
...
def main(argv: list) -> int:
    if len(argv) != 3:
        print("usage: normalize_scg_v80.py <input.csv> <output.csv>",
              file=sys.stderr)
        return 2
    ...
    if got != {"encryption": 2, "vami": 3}:
        raise SystemExit(f"ERROR: unexpected build-74 delta counts {got}")
    print(f"[normalize_scg_v80] reset-port rows moved to portgroup: {n}",
          file=sys.stderr)
    return 0
